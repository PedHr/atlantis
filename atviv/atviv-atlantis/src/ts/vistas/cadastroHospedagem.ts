...
export default class CadastroHospedagem {
    ...
    public renderizar(): string {
        ...
        const opcoesClientes = this.armazem.Clientes.map((cliente, index) => 
            `<option value="${index}">${cliente.Nome} (Doc: ${cliente.Documentos[0]?.Numero || 'N/A'})</option>`
        ).join('');

        const opcoesAcomodacoes = this.acomodacoesDisponiveis.map((acomodacao, index) => 
            `<option value="${index}">${acomodacao.NomeAcomadacao} - (Solteiro: ${acomodacao.CamaSolteiro}, Casal: ${acomodacao.CamaCasal})</option>`
        ).join('');
    ...
    private renderizarErro(mensagem: string): string {
        return `
    ...
    private realizarCheckin(evento: Event): void {
        evento.preventDefault();

        const selectCliente = document.getElementById('select-cliente') as HTMLSelectElement;
        const selectAcomodacao = document.getElementById('select-acomodacao') as HTMLSelectElement;
